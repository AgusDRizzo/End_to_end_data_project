...
def load_daily_data(**context):
    ...
    try:
        ...
        file_path = f"/opt/airflow/dags/daily_tasks/daily_CSV/MOCK_DATA_CONSUMPTIONS_{consumption_date.strftime('%Y-%m-%d')}.csv"
        logging.info(f"Reading data from {file_path}...")
        df = pd.read_csv(file_path)

        logging.info("Writing data to MySQL...")
        df.to_sql('consumptions', engine, if_exists='append', index=False)
