DROP VIEW IF EXISTS view_Total_Trans_Ct;
CREATE VIEW view_Total_Trans_Ct AS 
	SELECT 
		cli.client_id, COALESCE(sub.Total_Trans_Ct, 0) AS Total_Trans_Ct
	FROM 
		credit_card_db.clients AS cli
	LEFT JOIN 
		(
		SELECT 
			car.client_id, COUNT(con.consumption_id) AS Total_Trans_Ct
		FROM 
