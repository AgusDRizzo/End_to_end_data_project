DROP VIEW IF EXISTS Total_Amt_Chng_Q4_Q1;
CREATE VIEW Total_Amt_Chng_Q4_Q1 AS 
WITH consumption_Q4 AS (
	SELECT 
		car.client_id, SUM(con.consumption_amount) AS Q4
	FROM 
	...
	consumption_Q1 AS ( 
		SELECT 
		car.client_id, SUM(con.consumption_amount) AS Q1
	FROM 
		credit_card_db.consumptions AS con
	LEFT JOIN
		credit_card_db.cards AS car
	...
	GROUP BY 
		car.client_id)
SELECT 
	cli.client_id, change_amt.cociente AS Total_Amt_Chng_Q4_Q1
FROM
...
LEFT JOIN
	...
	ON 
		q1.client_id = q4.client_id ) AS change_amt
	ON 
		cli.client_id = change_amt.client_id
